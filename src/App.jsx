...
function PerfilTab({ usuario, onLogout, onUniversidad, esFundador, numeroRegistro }) {
  ...
  return (
    <div style={{ padding: '52px 16px 100px', background: 'var(--bg-primary)', minHeight: '100vh' }}>
      ...
      {/* Cambiar universidad */}
      <button onClick={() => onUniversidad('cambiar')} style={{ width: '100%', background: 'var(--bg-card)', border: '1px solid var(--color-border)', borderRadius: 16, padding: '14px 16px', color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' || usuario?.universidad === 'santotomas' ? 'white' : 'var(--color-primary)', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 10, textAlign: 'left' }}>
        🏫 Cambiar universidad
      </button>
                              ...
                              {bloquesDia.map(b => {
                                ...
                                return (
                                  <div key={b.id} onClick={e => { e.stopPropagation(); abrirEditar(b) }}
                                    ...
                                    <div style={{ fontSize: 9, fontWeight: 800, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' || usuario?.universidad === 'santotomas' ? 'white' : 'var(--color-primary)', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.ramo_nombre}</div>
                                    <div style={{ fontSize: 8, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' || usuario?.universidad === 'santotomas' ? 'white' : 'var(--color-primary)', opacity: 0.85, marginTop: 1 }}>{b.hora_inicio}–{b.hora_fin}</div>
                                    {b.sala && <div style={{ fontSize: 8, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' || usuario?.universidad === 'santotomas' ? 'white' : 'var(--color-primary)', opacity: 0.65, marginTop: 1 }}>{b.sala}</div>}
                                  </div>
                                )
                              })}
                            </div>
