  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', padding: '52px 16px 100px' }}>
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <button onClick={() => abrirNuevoBloque('Lunes', '')} style={{ width: '100%', marginBottom: 12, padding: '10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 12, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' ? 'white' : 'var(--color-primary)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                              ...
                              {bloquesDia.map(b => {
                                ...
                                return (
                                  <div key={b.id} onClick={e => { e.stopPropagation(); abrirEditar(b) }}
                                    ...
                                    <div style={{ fontSize: 9, fontWeight: 800, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' ? 'white' : 'var(--color-primary)', lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.ramo_nombre}</div>
                                    <div style={{ fontSize: 8, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' ? 'white' : 'var(--color-primary)', opacity: 0.85, marginTop: 1 }}>{b.hora_inicio}–{b.hora_fin}</div>
                                    {b.sala && <div style={{ fontSize: 8, color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' ? 'white' : 'var(--color-primary)', opacity: 0.65, marginTop: 1 }}>{b.sala}</div>}
                                  </div>
                                )
                              })}
                            </div>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ padding: '0 16px' }}>
          ...
          {mostrando ? (
            ...
            <button onClick={() => setMostrando(true)} style={{ width: '100%', background: 'rgba(255,255,255,0.08)', border: '1px solid var(--color-primary)', borderRadius: 16, padding: '16px', color: usuario?.universidad === 'uautonoma' || usuario?.universidad === 'inacap' ? 'white' : 'var(--color-primary)', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
              + Agregar ramo
            </button>
          )}
        </div>
